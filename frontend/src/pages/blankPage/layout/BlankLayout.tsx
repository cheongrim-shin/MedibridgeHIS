...
import { BlankSidebar } from './BlankSidebar.tsx';
...
export const BlankLayout = () => {
    ...
    const activeMenu = (lastPath && ADMIN_MENU_ITEMS.some((item) => item.id === lastPath))
        ? lastPath
        : 'patients';

    const currentMenu = ADMIN_MENU_ITEMS.find((item) => item.id === activeMenu) || ADMIN_MENU_ITEMS[0];

    return (
        <div className={styles.dashboardContainer}>
            {/* Admin Sidebar */}
            <BlankSidebar activeMenu={activeMenu} />
